...
> df_06_city_unique_spending <- df_06_city_spending %>% select(recipient_city_name) %>% group_by(recipient_city_name) %>% summarize(total_spending = sum(df_06$fed_funding_amount_06)) 

> df_07_unique_city_spending <- df_07_city_spending %>% select(recipient_city_name) %>% group_by(recipient_city_name) %>% summarize(total_spending = sum(df_07$fed_funding_amount_07)) 
